...
router.get('/events/:yr/:mth', (req, res) => {
	Event.month(Number(req.params.yr), Number(req.params.mth))
		.exec().then((events) => res.status(200).json(events)).catch((err) => res.status(400).json(err))
});

router.get('/events/:yr/:mth/:days', (req, res) => {
	var days = req.params.days.split('-')
	//var daysStart = days[0], daysEnd = days[1]
	Event.days(Number(req.params.yr), Number(req.params.mth), Number(days[0]), Number(days[1]))
		.exec().then((events) => res.status(200).json(events)).catch((err) => res.status(400).json(err))
});
